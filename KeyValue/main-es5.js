            ...
            if (json[i] instanceof Object) {
              ...
              if (!keyExists) {
                ...
                keyValues.map(function (element) {
                  if (element.key === key) {
                    element.values.set(fileName, json[i]);
                  }
                });
                keys.pop();
              }
    ...
    function () {
      ...
      _createClass(SelectedKeyValComponent, [{
        ...
        value: function saveSelectedItems() {
          ...
          var modifiedJsonToParse = this.updatedKeyValuePair.map(function (row) {
            var values = {};
          ...
          var finalJson = this.keyValueToJason.getJsonFromKeyValuePairs(modifiedJsonToParse);
        ...
        key: "onUpdate",
        value: function onUpdate(keyValuePair, keyChanged, event) {
          ...
          this.updatedKeyValuePair = data.map(function (row) {
            if (row.key === keyValuePair.key) {
              ...
              try {
                for (var _iterator3 = row.values[Symbol.iterator](), _step3; !(_iteratorNormalCompletion3 = (_step3 = _iterator3.next()).done); _iteratorNormalCompletion3 = true) {
                  var _step3$value = _slicedToArray(_step3.value, 2),
                      key = _step3$value[0],
                      value = _step3$value[1];

                  if (key === keyChanged) {
                    values.set(keyChanged, event);
                  } else {
