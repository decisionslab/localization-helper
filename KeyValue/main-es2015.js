...
let SelectedKeyValComponent = class SelectedKeyValComponent {
    ...
    saveSelectedItems() {
        ...
        let modifiedJsonToParse = this.updatedKeyValuePair.map(row => {
            let values = {};
            for (let [key, value] of row.values) {
                values[key] = value;
            }
            return {
                key: row.key,
        ...
        const finalJson = this.keyValueToJason.getJsonFromKeyValuePairs(modifiedJsonToParse);
    ...
    onUpdate(keyValuePair, keyChanged, event) {
